    ...
    try {
      if (data.mailID && data.password) {
        let user = await axios.post(
          "https://real-estate-catalog-gp8x.onrender.com/login",
          data
        );
        if (user.data.token) {
        ...
        <div id="container">
          ...
          <div className="inp-field flexing">
            <input
              ...
              placeholder="User ID"
              value={data.mailID}
              onChange={(e) => setData({ ...data, mailID: e.target.value })}
            />
            <input
              type={show}
              className="inp"
            ...
            />
            <span>
              {<VisibilityOffIcon className="eye" onClick={handlepass} />}
            </span>
          </div>
          <div className="btn-field flexing">
            ...
            <span className="signup font" onClick={handleReg}>
              Sign Up
            </span>
          </div>
