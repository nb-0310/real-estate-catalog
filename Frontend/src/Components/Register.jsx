...
const Register = () => {
  ...
  const registers = async () => {
    let verify =
      ...
      val.mailID.length !== 0 &&
      val.confirmpassword.length !== 0;
    if (
      val.password.length >= 5 &&
      val.password.length <= 10 &&
      ...
      try {
        ...
        let data = await axios.post(
          "https://real-estate-catalog-gp8x.onrender.com/register",
          val
        );
  ...
  if (loader) {
    return (
      ...
      >
        <img
          src={
            "https://cdn.dribbble.com/users/241526/screenshots/954930/loader.gif"
          }
          alt="loading-gif"
          width="30%"
          height="30%"
      ...
      <div id="container">
        ...
        <div className="inp-field flexing">
          <input
            ...
            className="inp"
            placeholder="Mail ID"
            onChange={(e) => setVal({ ...val, mailID: e.target.value })}
            value={val.mailID}
          />
          <input
            type="text"
        ...
        <div className="btn-field flexing">
          <button id="signin" onClick={registers}>
            Sign up
          </button>
        </div>
        ...
        {passwordError && (
          <div className="err-1">
            *password must be in the range of 5-10 characters
          </div>
        )}
        {val.password.length !== 0 && val.password !== val.confirmpassword ? (
          <div className="err-1">The password must be same</div>
        ) : null}
      </div>
      <div>
        <p>
